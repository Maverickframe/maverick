...
<div class="js-contacts-form-container contacts-form-card">
            <form action="" method="POST" class="js-contacts-form cform" enctype="multipart/form-data" novalidate>
                ...
                <div class="cform__row">
                    <label class="cform__input">
                        ...
                        <input type="text" name="Name" maxlength="80" placeholder="<?php echo esc_attr(mfs_t('Full name', 'Nombre completo')); ?>*" required>
                        <span class="cform__error"><?php echo mfs_t('Enter your name', 'Introduce tu nombre'); ?></span>
                    </label>

                    <label class="cform__input">
                        <span class="sr-only">Email</span>
                        <input type="email" name="Email" maxlength="120" placeholder="Email*" required>
                        <span class="cform__error"><?php echo mfs_t('It is not email', 'El correo no es válido'); ?></span>
                    </label>
                </div>

                <div class="cform__input cform__phone">
                    <div class="cform__country js-country" data-dial="+34">
                        <button type="button" class="cform__country-toggle js-country-toggle" aria-haspopup="listbox" aria-expanded="false" aria-label="<?php echo esc_attr(mfs_t('Country code', 'Código de país')); ?>">
                        ...
                        <div class="cform__country-pop">
                            <input type="text" class="cform__country-search js-country-search" placeholder="<?php echo esc_attr(mfs_t('Search country…', 'Buscar país…')); ?>" aria-label="<?php echo esc_attr(mfs_t('Search country', 'Buscar país')); ?>">
                    ...
                    <label class="cform__phone-field">
                        <span class="sr-only"><?php echo mfs_t('Phone number', 'Número de teléfono'); ?></span>
                        <input type="tel" class="cform__phone-num" inputmode="tel" autocomplete="tel-national" maxlength="20" placeholder="<?php echo esc_attr(mfs_t('Phone number', 'Número de teléfono')); ?>">
                        <span class="cform__error cform__phone-error"><?php echo mfs_t('Enter a valid phone number', 'Introduce un número de teléfono válido'); ?></span>
                    </label>
                    <input type="hidden" name="Phone">
                </div>

                <label class="cform__input">
                    <span class="sr-only"><?php echo mfs_t('How can we help you?', '¿Cómo podemos ayudarte?'); ?></span>
                    <textarea name="Message" rows="4" maxlength="1000" class="js-msg" placeholder="<?php echo esc_attr(mfs_t('How can we help you?', '¿Cómo podemos ayudarte?')); ?>"></textarea>
                ...
                <div class="cform__upload-row">
                    <label class="cform__upload">
                        ...
                        <span><?php echo mfs_t('Upload file', 'Adjuntar archivo'); ?></span>
                    </label>
                    <span class="cform__upload-info" tabindex="0" role="button" aria-label="<?php echo esc_attr(mfs_t('File requirements', 'Requisitos del archivo')); ?>">
                        ...
                        <span class="cform__upload-tip" role="tooltip">
                            <?php echo mfs_t(
                                ...
                                'Tamaño máximo: 15 MB. Formatos: PDF, JPG, PNG, ZIP.'
                            ); ?>
                        </span>
                    </span>
                ...
                <label class="cform__check">
                    ...
                    <span><?php echo mfs_t('I agree to the', 'Acepto la'); ?>
                        <a href="<?php echo esc_url($privacy_url); ?>" target="_blank" rel="noopener"><?php echo mfs_t('Privacy Policy', 'Política de privacidad'); ?></a>
                        <?php echo mfs_t('and give my permission to process my personal data for the purposes specified in the Privacy Policy.', 'y doy mi consentimiento para tratar mis datos personales con los fines indicados en la Política de privacidad.'); ?>*
                    </span>
                    <span class="cform__error"><?php echo mfs_t('Please accept the Privacy Policy', 'Debes aceptar la Política de privacidad'); ?></span>
                </label>

                <label class="cform__check">
                    <input type="checkbox" name="NewsletterConsent" value="1">
                    <span><?php echo mfs_t('I give my permission to process my email address specified above for the purpose of sending newsletters.', 'Doy mi consentimiento para tratar el correo indicado con el fin de enviarme novedades.'); ?></span>
                </label>

                <button class="btn-main fill cform__btn" type="submit"><?php echo mfs_t('Submit', 'Enviar'); ?></button>
            </form>
            ...
            <div class="cform__success contacts-form__success">
                <div class="cform__success-card">
                    ...
                    <button type="button" class="cform__success-close" aria-label="<?php echo esc_attr(mfs_t('Close', 'Cerrar')); ?>" onclick="this.closest('.js-contacts-form-container').classList.remove('is-success')">
                        <?php echo inline_svg('icons/close-white.svg') ?: '&times;'; ?>
                    </button>
                    <p class="cform__success-text">
                        <span class="cform__success-hl"><?php echo mfs_t('Thank you – your message has been sent.', 'Gracias, tu mensaje se ha enviado.'); ?></span>
                        <?php echo mfs_t('Our team will review your request and get back to you shortly.', 'Nuestro equipo revisará tu solicitud y te responderá en breve.'); ?>
                    </p>
                </div>
            </div>
</div>
